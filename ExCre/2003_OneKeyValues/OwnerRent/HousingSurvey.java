...
public class HousingSurvey{
   ...
   public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text> {
      ...
      public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
         ...
         String ownOrRent = row[19];
         
         //values
         double income = Double.parseDouble(row[13]);
         int ageOfHouseHold = Integer.parseInt(row[1]); 
         int numberOfPeople = Integer.parseInt(row[12]);
        
          //Key Value for OwnerandRent     
         keyOfMap.set(ownOrRent);
